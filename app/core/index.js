...
class Context extends EE {
  ...
  constructor(url, canvas, options = {}) {
    ...
    debug(
      `Context with url "${this.#url}", runtime id: "${this.#id}" initialized`
    );
  ...
  setUrl(url = CONTEXT_URL_BASE, autoCreateLayers = CONTEXT_AUTOCREATE_LAYERS) {
    if (!url || typeof url !== "string")
      throw new Error(
        `Context url must be of type string, "${typeof url}" given`
      );
    ...
    debug(
      `Setting context url for context id "${this.#id}" to "${parsed.url}"`
    );
    if (!this.#tree.insert(parsed.path, null, autoCreateLayers)) {
      debug(
        `Context url "${parsed.url}" not set, path "${parsed.path}" not found`
      );
      return false;
    }
  ...
  async removeDocument(id) {
    debug(
      `Removing document with id "${id}" from context "${this.#id}, url "${this.#url}"`
    );
  ...
  #initializeLayers(
    ...
    layerArray.forEach((layerName) => {
      let initialized =
        !this.#layerIndex.hasLayerName(layerName) && autoCreateLayers
          ? this.createLayer(layerName)
          : this.#layerIndex.getLayerByName(layerName);

      if (!initialized) {
        debug(
          `Layer "${layerName}" not found and autoCreateLayers is set to false, skipping initialization`
        );
        return; // TODO: FIXME
      }
